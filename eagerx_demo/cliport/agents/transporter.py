...
from eagerx_demo.cliport.models.streams.two_stream_attention import TwoStreamAttention
from eagerx_demo.cliport.models.streams.two_stream_transport import TwoStreamTransport

from eagerx_demo.cliport.models.streams.two_stream_attention import TwoStreamAttentionLat
from eagerx_demo.cliport.models.streams.two_stream_transport import TwoStreamTransportLat
...
class TransporterAgent(LightningModule):
    ...
    def attn_criterion(self, backprop, compute_err, inp, out, p, theta):
        ...
        if compute_err:
            ...
            err = {"dist": np.linalg.norm(np.array(p) - p0_pix, ord=1), "theta": np.absolute((theta - p0_theta) % np.pi)}
        return loss, err
    ...
    def transport_criterion(self, backprop, compute_err, inp, output, p, q, theta):
        ...
        if compute_err:
            ...
            err = {"dist": np.linalg.norm(np.array(q) - p1_pix, ord=1), "theta": np.absolute((theta - p1_theta) % np.pi)}
