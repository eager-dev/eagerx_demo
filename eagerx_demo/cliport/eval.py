...
from eagerx_demo.cliport import dataset
from eagerx_demo.cliport import tasks
from eagerx_demo.cliport.utils import utils
from eagerx_demo.cliport.environments.environment import Environment
...
def main(vcfg):
    ...
    if "multi" in dataset_type:
        ds = dataset.RavensMultiTaskDataset(
            vcfg["data_dir"], tcfg, group=eval_task, mode=mode, n_demos=vcfg["n_demos"], augment=False
        )
    else:
        ds = dataset.RavensDataset(
            os.path.join(vcfg["data_dir"], f"{eval_task}-{mode}"), tcfg, n_demos=vcfg["n_demos"], augment=False
        )
