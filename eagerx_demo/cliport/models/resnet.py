...
class IdentityBlock(nn.Module):
    def __init__(self, in_planes, filters, kernel_size, stride=1, final_relu=True, batchnorm=True):
        ...
        self.conv2 = nn.Conv2d(filters1, filters2, kernel_size=kernel_size, dilation=1, stride=stride, padding=1, bias=False)
...
class ResNet43_8s(nn.Module):
    ...
    def _make_layers(self):
        layers = nn.Sequential(
            ...
            nn.UpsamplingBilinear2d(scale_factor=2),
            # conv2
            ConvBlock(64, [16, 16, self.output_dim], kernel_size=3, stride=1, final_relu=False, batchnorm=self.batchnorm),
            IdentityBlock(
...
class DummyResNet43_8s(nn.Module):
    def __init__(self, input_shape, output_dim, cfg, device, preprocess):
        super(DummyResNet43_8s, self).__init__()
        self.input_shape = input_shape
        self.input_dim = input_shape[-1]
        self.output_dim = output_dim
        self.cfg = cfg
