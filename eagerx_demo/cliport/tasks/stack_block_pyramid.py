...
class StackBlockPyramid(Task):
    ...
    def reset(self, env):
        ...
        self.goals.append((objs[:3], np.ones((3, 3)), targs[:3], False, True, "pose", None, 1 / 2))
        self.lang_goals.append(self.lang_template.format(blocks="the green, blue and purple blocks", row="bottom"))

        # Goal: blocks are stacked in a pyramid (middle row: yellow, orange).
        self.goals.append((objs[3:5], np.ones((2, 2)), targs[3:5], False, True, "pose", None, 1 / 3))
        self.lang_goals.append(self.lang_template.format(blocks="the yellow and orange blocks", row="middle"))

        # Goal: blocks are stacked in a pyramid (top row: red).
        self.goals.append((objs[5:], np.ones((1, 1)), targs[5:], False, True, "pose", None, 1 / 6))
        self.lang_goals.append(self.lang_template.format(blocks="the red block", row="top"))
