...
class CameraSensor(EngineNode):
    ...
    def initialize(self, spec: NodeSpec, simulator: Dict):
        ...
        self.cam_cb = self._camera_measurement(self._p, self.mode, self.cb_args, self.intrinsic)
    ...
    def callback(self, t_n: float, tick: Msg = None, pos: Msg = None, orientation: Msg = None):
        ...
        color, depth = self.cam_cb()
        msg = dict(color=color, depth=depth) if "d" in self.mode else dict(color=color)
        return msg
    ...
    @staticmethod
    def _camera_measurement(p, mode, cb_args, intrinsic):
        if mode in ["rgb", "bgr"]:

            def cb():
                _, _, rgba, _, _ = p.getCameraImage(**cb_args)
            ...
            def cb():
                _, _, rgba, depth, _ = p.getCameraImage(**cb_args)
                rgba = rgba if mode == "rgbd" else cv2.cvtColor(rgba, cv2.COLOR_RGBA2BGRA)
                depth_image_size = (cb_args["height"], cb_args["width"])
                zbuffer = np.array(depth, dtype="float32").reshape(depth_image_size)
                zfar = intrinsic["farVal"]
                znear = intrinsic["nearVal"]
